...
if __name__ == "__main__":
    ...
    if not opt.resume:
        if opt.train_from_scratch:
            ...
            pretrained_state_dict = torch.load(opt.pretrained_model, map_location='cpu')['state_dict']
            model_state_dict = model.state_dict()
            new_state_dict = {}
            for name, param in pretrained_state_dict.items():
                # print(name)
                if name.startswith('model.diffusion_model.input_blocks.0.0') or name.startswith('model.diffusion_model.out.2'):
                    continue  # 跳过不需要加载的权重
